...
module.exports = function (player, tile, index, item) {
  ...
  // Create the new item
  let newThing = process.gameServer.database.createThing(Number(lookup[item.id]));

  // Copy duration from the original item if it exists
  // This preserves the remaining "fuel" when toggling torches
  if (item.duration !== undefined && item.duration > 0) {
    newThing.setDuration(item.getRemainingDuration());
  }

  // Replace the items
  item.replace(newThing);
