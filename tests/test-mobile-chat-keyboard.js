...
const styleValues = {};
const chatContainer = {
  classList: createClassList(["mobile-chat-active"]),
  style: {
...
const documentMock = {
  activeElement: input,
  getElementById(id) {
    return id === "chat-input" ? input : null;
  },
...
assert.strictEqual(styleValues["--mobile-chat-viewport-top"], "142px");
assert.strictEqual(styleValues["--mobile-chat-viewport-height"], "104px");
...
assert(chatContainer.classList.contains("mobile-chat-keyboard-tiny"));
assert.strictEqual(styleValues["--mobile-chat-viewport-top"], "30px");
assert.strictEqual(styleValues["--mobile-chat-viewport-height"], "58px");
